...
def show_items(category_name):
    ...
    session.close()
    if not items:
        flash("There are currently no Courses in this category.")
...
def show_my_items():
    ...
    if not items:
        flash("You haven't add any Courses yet.")
...
def create_item():
    ...
    if request.method == 'POST':
        if not request.form['name']:
            flash("New Course not created: No name provided.")
            return redirect(url_for('show_homepage'))

        if request.form['name'] == "items":
            # Can't have an item called "items" as this is a route.
            flash("Error: Can't have an Course called 'items'.")
            return redirect(url_for('show_homepage'))
        ...
        if already_exists is True:
            flash("Error: There is already an Course with the name '%s'"
                  % request.form['name'])
            session.close()
            return redirect(url_for('show_homepage'))
        ...
        flash("New Course successfully created!")
...
def edit_item(item_name, category_name=None):
    ...
    """
    if 'username' not in login_session:
        flash("You need to login to edit any Courses.")
        return redirect('/login')
    ...
    if login_session['user_id'] != item.user_id:
        flash("You didn't add this Course, so you can't edit it. Sorry :-(")
